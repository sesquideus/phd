...
\author{\small Martin Baláž \\ Juraj Tóth, PhD. \\ Robert Jedicke, PhD. \\ Peter Vereš, PhD.}
...
\begin{document}
			...
			\frejm{Algorithm}{
				\begin{enumerate}
					...
					\item apply observational bias 
						...
						\end{itemize}
                    \pause
					\item calculate the statistic and compare it with AMOS data 
					\item adjust the particle distribution and bias parameters 
					\pause
					\item repeat
				\end{enumerate}
			}
        ...
        \subsection{Simulation}
            \frejm{Model}{            
                ...
                \begin{itemize}
                    ...
                    \item to find out how the cameras see meteors
            ...
            \frejm{Selection bias}{
                ...
                \begin{itemize}
                    ...
                    \begin{itemize}
                        \item decide if it is detected
                        \item based on properties of the brightest frame
                        \item compare the resulting set to observation
                    \end{itemize}
                \end{itemize}
            }
    ...
    \section{Asmodeus}    
		\subsection{Asmodeus}
            ...
            \frejm{What is it good for}{
                A multi-purpose tool for statistical evaluation of meteor populations
                \begin{itemize}
                    \item generate a meteoroid population
                    \item observe virtual meteors
                \end{itemize}
                \pause

                We are able to
                \begin{itemize}
                    \item estimate particle density and flux
                    \item capture all relevant parameters
                    \item examine correlations
                \end{itemize}
                \pause
                Everything will be shared on \emph{GitHub}
            }
        
            \fspicture{mjd-entryAngle-appMag.png}
            \fspicture{logInitMass-elevation-appMag.png}
    ...
    \section{Flux}
        \subsection{Flux}
            ...
            \frejm{Heatmap}{
                \hspace*{-8mm}%
                \includegraphics[width = \paperwidth]{chiSquare-magnitude-1d8.pdf}
            }
            \frejm{Heatmap -- zooming in}{
                \hspace*{-8mm}%
                \includegraphics[width = \paperwidth]{chiSquare-magnitude-1d8z.pdf}
            }
			...
			\frejm{Summary}{
				\begin{itemize}
					...
					\begin{itemize}
						...
						\item also aesthetically pleasing
					\end{itemize}
				\end{itemize}
				\pause
				\begin{itemize}
					...
					\begin{itemize}
						\item values are not perfect
						\item mass index seems to be much higher than known values
					\end{itemize}
				\end{itemize}
                \pause
				\begin{itemize}
                    \item the tool will be shared and free to use

...
class Vector3D:
    ...
    def __xor__(self, other):
        ...
        if isinstance(other, Vector3D):
            return __class__.from_numpy_vector(np.cross(other.np(), self.np()))
        else:
    ...
    def from_geodetic(cls, lat, lon, alt=0):
        ...
        """
        return cls.from_spherical(lat, lon, alt + 6371000)
    ...
    def altaz_to_dxdydz(self, local: 'Vector3D') -> 'Vector3D':
        ...
        """
        return Vector3D.from_numpy_vector(self.rotation_matrix() @ local.np())

    def dxdydz_to_altaz(self, ecef: 'Vector3D') -> 'Vector3D':
        """Transform vector `ecef` to local observer's alt-az coordinate system"""
        return Vector3D.from_numpy_vector(self.derotation_matrix() @ ecef.np())

    def altaz_to_dxdydz_at(self, location: 'Vector3D') -> 'Vector3D':
        return Vector3D.from_numpy_vector(location.rotation_matrix() @ self.np())
    
    def dxdydz_to_altaz_at(self, location: 'Vector3D') -> 'Vector3D':
        return Vector3D.from_numpy_vector(location.derotation_matrix() @ self.np())

...
\NewDocumentCommand{\integrate}{O{} O{} m m}{\kern -0.2em \int\limits_{#1}^{#2} #3 \, \diff#4}
\NewDocumentCommand{\iintegrate}{O{} O{} m m m}{\iint\limits_{#1}^{\quad#2} #3 \diff#4\diff#5}
\NewDocumentCommand{\iiintegrate}{O{} O{} m m m m}{\iiint\limits_{#1}^{#2} #3 \diff#4\diff#5\diff#6}
...
\NewDocumentCommand{\magnitude}{m}{{#1}^{\mathrm{m}}}
...
\NewDocumentCommand{\fig}{O{} m m m +m}{
    \begin{figure}[H]
        \centering
        \includegraphics[keepaspectratio = true, width = #3]{pictures/#2}
        \caption[#1]{#5}
        \label{#4}
    \end{figure}
}
...
\setlength{\epigraphwidth}{0.333\textwidth}

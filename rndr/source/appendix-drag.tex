...
\section{Singh model} \label{DS}
    ...
    free-flow temperature $T_\infty$ and 

    Furthermore, the correlation requires several constants:
    \begin{itemize}
        \item the \emph{ratio of heat capacities} $\gamma \equiv \frac{c_p}{c_V}$,
            whose value for air is $7/5 = \num{1.4}$;
        \item $\omega$, the exponent in the power law dependence
            of viscosity on temperature ($\mu \propto T^\omega$).
            For air, one such approximate correlation \citep{tec-science-viscosity}
    ...
    Further, $W_r$ is corrected for supersonic speeds as
    \eqn{eq:DS-WrT}{
        W_r^T =
        \begin{dcases}
            W_r & M_\infty \leq 1, \\
    ...
    where $\Theta$ is a function of Mach number $M$:
    \eqn{eq:DS-Ym}{
    ...
    The expression for free-molecular flow is the same as in the Loth model (see \cref{eq:DL-Gfm}),
    \eqn{eq:DS-Gfm}{
        \Gamma_\mathrm{fm} =
            \frac{\left(1 + 2s^2\right) \exp(-s^2)}{s^3 \sqrt{\pi}} +
            \frac{\left(4s^4 + 4s^2 - 1\right) \erf(s)}{2 s^4} +
    ...
    For low $M_\infty$, a rarefaction correction is computed,
    \eqn{eq:DS-fKn}{
    ...
    where the bridging modulator $\eta$ is set to $\eta = \num{1.8}$.

...
\subsection{Models of meteoroid dynamics} \label{mm}
    ...
    \subsubsection{Material evaporation model} \label{mwh}
        ...
        by the chain rule $\Derivative{\left(\ln m\right)}{t} = \frac{1}{m} \Derivative{m}{t}$.
        The second term then can be separated and the whole equation \ref{eq:heh} rewritten as
        \eqn{eq:hrh-fixed}{
            \Derivative{T}{t} = \frac{1}{c} \left(
...
%                well even when all other forces are neglected.
                ...
                \eqn{eq:sai-totalforce}{
                    \vec{F} = \vec{F_\mathrm{G}} + \vec{F_\mathrm{Coriolis}} + \vec{F_\mathrm{Huygens}} + \vec{F_\mathrm{d}}
                }
%
